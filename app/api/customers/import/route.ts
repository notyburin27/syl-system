  ...
  try {
    ...
    rows.forEach((row, index) => {
      ...
      if (!row.name || !row.name.trim()) {
        errors.push({ row: rowNum, field: "name", message: "กรุณาระบุชื่อลูกค้า" });
        return;
      }

      if (existingNames.has(row.name)) {
        errors.push({ row: rowNum, field: "name", message: `ชื่อลูกค้าซ้ำในระบบ: ${row.name}` });
      }

      const dups = nameCount.get(row.name);
      if (dups && dups.length > 1) {
        errors.push({ row: rowNum, field: "name", message: `ชื่อลูกค้าซ้ำในไฟล์ (แถว ${dups.join(", ")})` });
      }
    });
    ...
    const created = await prisma.$transaction(
      rows.map((row) =>
        prisma.customer.create({ data: { name: row.name.trim() } })
      )
    );
    ...
    if (error && typeof error === "object" && "code" in error && error.code === "P2002") {
      return NextResponse.json({ error: "ชื่อลูกค้าซ้ำในระบบ" }, { status: 400 });
    }
