...
const VALID_TYPES = ["factory", "general"];
  ...
  try {
    ...
    rows.forEach((row, index) => {
      ...
      if (!row.name || !row.name.trim()) {
        errors.push({ row: rowNum, field: "name", message: "กรุณาระบุชื่อสถานที่" });
        return;
      }
      ...
      if (!row.type || !VALID_TYPES.includes(row.type)) {
        errors.push({
          row: rowNum,
          field: "type",
          message: `ประเภทไม่ถูกต้อง: ${row.type || "(ว่าง)"} (ใช้ factory หรือ general)`,
        });
      }

      if (existingNames.has(row.name)) {
        errors.push({ row: rowNum, field: "name", message: `ชื่อสถานที่ซ้ำในระบบ: ${row.name}` });
      }

      const dups = nameCount.get(row.name);
      if (dups && dups.length > 1) {
        errors.push({ row: rowNum, field: "name", message: `ชื่อสถานที่ซ้ำในไฟล์ (แถว ${dups.join(", ")})` });
      }
    });
    ...
    const created = await prisma.$transaction(
      rows.map((row) =>
        prisma.location.create({
          data: { name: row.name.trim(), type: row.type },
        })
      )
    );
    ...
    if (error && typeof error === "object" && "code" in error && error.code === "P2002") {
      return NextResponse.json({ error: "ชื่อสถานที่ซ้ำในระบบ" }, { status: 400 });
    }
