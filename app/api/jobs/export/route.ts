...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { leavesByDriver, holidays } = await fetchBannerSources(monthStr, [driverId]);
    const banners = buildBannersFor(monthStr, jobs, leavesByDriver.get(driverId) ?? [], holidays);
